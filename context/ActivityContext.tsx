...
import { ACTIVITIES_QUERY, ACTIVITY_QUERY } from "@/constant/Query";
/**
...
export function ActivityContextProvider({ children }: { children: ReactNode }) {
  ...
  const fetchActivitiesDB = async () => {
    const rows = await DB.query(ACTIVITIES_QUERY, ["ACTIVE"]);
  ...
  const fetchActivityDB = async (id: string) => {
    const row = await DB.first(ACTIVITY_QUERY, ["ACTIVE", id]);
